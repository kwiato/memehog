...
4. **Web UI port** — default `8080`.

It generates an `API_TOKEN`, writes `.env` and starts the stack. Open `http://<pi-address>:8080` and send your bot a meme.
...
**Web UI** — browse, search (`kot w kapeluszu` matches prefixes, so partial words work), filter by type/tag, click a meme for the lightbox where you can tag, download or delete it. The **＋ Add** button uploads files or queues a link for download.
...
Memehog has no login page — it's designed to sit on a private network. The recommended setup is [Tailscale](https://tailscale.com/): install it on the Pi and your devices, then browse `http://<pi-tailnet-name>:8080` from anywhere. Only the `/api/v1/*` endpoints are additionally protected with the Bearer token, so automations (and the future browser extension) can authenticate.
...
curl -X POST http://pi:8080/api/v1/jobs \
  -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
  -d '{"url": "https://www.tiktok.com/@user/video/123"}'

# search
curl "http://pi:8080/api/v1/items?q=cat" -H "Authorization: Bearer $TOKEN"

# upload a file
curl -X POST http://pi:8080/api/v1/items \
  -H "Authorization: Bearer $TOKEN" -F "file=@meme.jpg"
```

Interactive docs: `http://pi:8080/api/docs`.
...
python -m memehog      # http://localhost:8080
pytest
```
